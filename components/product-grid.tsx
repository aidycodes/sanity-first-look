...
export function ProductGrid({products}: Props) {
  ...
  return (
    <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-3 lg:col-span-3 lg:gap-x-8">
      {products.map(({name, price, images, _id, slug, currency  }) => {
        console.log(images[0], _id)
        if(_id === 'e882fe48-253c-40fb-8155-51b47b063c1a') return null
        return(
        <Link key={_id} href={`/products/${slug}`} className="group text-sm">
          ...
          <p className="mt-2 font-medium">{formatCurrencyString({currency: currency, value: price})}</p>
        </Link>
 )} )}
    </div>
  )
